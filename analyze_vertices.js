// Script para analisar conexões entre vértices no Catan
...
// Encontra conexões entre vértices (através das arestas)
const vertexConnections = new Map();

verticesMap.forEach(vertex => {
  vertexConnections.set(vertex.position, new Set());
});

// Para cada aresta, adiciona a conexão entre os dois vértices
edgesMap.forEach(edge => {
  const pos1 = edge.v1.position;
  const pos2 = edge.v2.position;
  vertexConnections.get(pos1).add(pos2);
  vertexConnections.get(pos2).add(pos1);
});

console.log('=== CONEXÕES ENTRE VÉRTICES (POSIÇÕES) ===\n');

vertexConnections.forEach((connections, position) => {
  const sortedConnections = Array.from(connections).sort((a, b) => a - b);
  console.log(`Posição ${position}: conectado a ${sortedConnections.join(', ')}`);
});

console.log('\n=== VERIFICAÇÃO ESPECÍFICA ===');
console.log(`Posição 6 está conectada a: ${Array.from(vertexConnections.get(6)).sort((a, b) => a - b).join(', ')}`);
console.log(`Posição 5 está conectada a: ${Array.from(vertexConnections.get(5)).sort((a, b) => a - b).join(', ')}`);
console.log(`Posição 9 está conectada a: ${Array.from(vertexConnections.get(9)).sort((a, b) => a - b).join(', ')}`);
console.log(`Posição 11 está conectada a: ${Array.from(vertexConnections.get(11)).sort((a, b) => a - b).join(', ')}`);

console.log('\n=== RESPOSTA À PERGUNTA ===');
const pos6Connections = vertexConnections.get(6);
console.log(`Da posição 6, você pode construir estradas diretamente para:`);
console.log(`- Posição 5: ${pos6Connections.has(5) ? 'SIM' : 'NÃO'}`);
console.log(`- Posição 9: ${pos6Connections.has(9) ? 'SIM' : 'NÃO'}`);
console.log(`- Posição 11: ${pos6Connections.has(11) ? 'SIM' : 'NÃO'}`);
